...
### Added
- **`FORCE=1` re-pulls the full price history** instead of only the days since each product's cached `asOf`. The cache is incremental by design, which also makes bad source data permanent — nothing ever re-asks for a day it already has. Forcing overlays a full re-pull on the cached series, so days the API still serves get corrected while days it no longer returns keep their cached value. Honoured by both `fetch-price-history-shard.mjs` and `update-supermarket-deals.mjs`, and exposed as a `force` dispatch input on both workflows (`gh workflow run update-price-history-sharded.yml --ref master -f force=true`). Recovery tool, not a routine setting. **Run it locally** (`FORCE=1 node scripts/update-supermarket-deals.mjs`, ~1h): a forced sharded run on 2026-07-26 timed out all 10 shards at the 25-minute cap and committed nothing, because a full-range query is far heavier than the daily delta and e-kalathi does not serve those to GitHub runner IPs — the same asymmetry as Bazaraki. Note that such a run still reports success (`continue-on-error`) and still reads as 100% cache-fresh, so verify the cache diff is non-empty.

### Fixed
- **All-time lows no longer show €0.00.** e-Kalathi reports a price of `0` for a day when a product is not listed anywhere, and `detectAllTimeLow` took that as a genuine record low of €0.00 at -100% off. On 2026-07-26 the source returned 0 for 145 of 476 products, so all 20 all-time-low slots filled with free groceries. Non-positive prices are now dropped at ingestion (`fetchPriceHistory`) and when expanding cached change-points (`expandPoints`), so the previous price simply carries through the gap, and `detectAllTimeLow` guards against a non-positive minimum the way `detectNearLow` always did. `fetch-price-history-shard.mjs` — the path the nightly sharded job actually ingests through — gets the same two filters, so the shards can't write zeros back into the merged cache.
...
### Rollback
Redeploy `v1.8.0` tag via Cloudflare Pages dashboard, or revert the relevant merge commits and push to master.
