...
export function mergeSouvlaki(wolt, extras) {
  ...
  // not erase the Bolt/Foody venues from the merged file
  const cityDefs = [];
  ...
  const woltByKey = new Map((wolt.cities || []).map((c) => [c.key, c.venues]));
  return {
    ...
    cities: cityDefs.map(({ key, label }) => {
      let merged = (woltByKey.get(key) || []).map((v) => ({ ...v, platforms: ["wolt"] }));
      ...
      return { key, label, venues: merged };
    }),
  };
}
